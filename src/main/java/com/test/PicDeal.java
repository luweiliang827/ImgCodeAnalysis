...
public class PicDeal {
    ...
    //todo splitNums可以根据你给到的图片色差进行调整，在你自己使用时，可以针对splitNums做一个循环，
    // 每次加多少，得到不同的色差比的二值化后的图片，因为不同的图片可能干扰线、干扰点颜色原因，二值化后会有差异
    ...
    //todo 分割图片
    public static void splitPic(String picFile,String resultUrl) throws Exception{
        //todo 分割图片
//        BufferedImage img = ImageIO.read(new File(picFile));
        ...
        for (int i = 0; i < splitWidthNum; i++) {
            ...
            for (int j=beginNum;j<=endNum;j+=25000){
                ...
                //todo 分割后图片写出去
                ImageIO.write(imgDeal, "JPG", new File(resultUrl));
            }
        }
    }
    ...
    public static BufferedImage removeBackgroud(Image img2,int j,int i)throws Exception {
        ...
        //todo 以下是对图片进行二值化处理，在这里我的思路是规定，色差范围在splitNums到负splitNums之间的，算是同色，
        // 放入同一个色值，放入一个map中，
